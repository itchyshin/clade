...
clade is a **forward-simulation testbed for behavioural and evolutionary
theory**, with three uses:
...
clade is NOT for statistical inference from real data. Pair with `lme4`,
`glmmTMB`, `brms`, or others when that’s the need.

Full vision, user list, lab values, and contracts:
...
- **Julia side** — `inst/julia/src/`. Currently under parallel review on
  Sergio’s branch (`claude/v0.8-core`, PR \#122) for the subtractive
  v0.8-core reset. **Do not touch `inst/julia/src/` without coordination
  with Sergio’s track** unless the change is purely additive and the
  user explicitly approves.
- **Boundary** — crossed exactly once per
  [`run_alife()`](https://itchyshin.github.io/clade/reference/run_alife.md)
  call via `JuliaConnectoR`. Never per tick. This is a contract — don’t
  introduce per-tick R↔︎Julia calls.

When an R-side change implies a Julia-side change (or vice versa), say
...
a commitment we have to test, document, and maintain forever, and
...
In this repo: audit claims need ≥5 seeds +
...
2.  Do not change public function signatures, spec field names, or
    ...
    defaults without updating the relevant Core Contract row in
    `dev/design/00-vision.md` AND adding a note in
...
examples, check-log entry, after-task note, and review are all present.
If one of those is not appropriate for the specific change, the
after-task report must say why.
...
| `R/config.R` | [`default_specs()`](https://itchyshin.github.io/clade/reference/default_specs.md), preset families |
| `R/scenarios.R` | Pre-baked scenario specs (`wolf_personality_specs`, etc.) |
| `R/hypothesis.R` | [`hypothesis_sweep()`](https://itchyshin.github.io/clade/reference/hypothesis_sweep.md) + [`hypothesis_report()`](https://itchyshin.github.io/clade/reference/hypothesis_report.md) |
| `R/analysis.R` | [`get_run_data()`](https://itchyshin.github.io/clade/reference/get_run_data.md), [`viability_report()`](https://itchyshin.github.io/clade/reference/viability_report.md), [`compute_ld()`](https://itchyshin.github.io/clade/reference/compute_ld.md), heritability, relatedness |
...
- Public R API surface is everything listed in `NAMESPACE`
  (`export(...)`).
- Specs are plain named lists —
  [`default_specs()`](https://itchyshin.github.io/clade/reference/default_specs.md)
  is the schema; `vignettes/parameter-reference.Rmd` is the user-facing
  reference (introspection-generated as of 0.7.1).
...
- Quantitative claims in paper-reproduction vignettes need ≥5 seeds with
  `mean ± SE`. Single-seed numbers are diagnostic only, labelled as
  such.
...
claim grounded in files, commands, tests, citations, or explicit design
assumptions. Six roles are standing; the others from the kit
...
Use the canonical names when reporting team perspectives. Don’t rename.
Don’t use names as decoration — if you name a role, attach a specific
question and a specific output.

## After-Task Protocol

Every meaningful task closes with an after-task report following
`dev/design/10-after-task-protocol.md`. Use the `after-task-audit` skill
(`.agents/skills/after-task-audit/SKILL.md`) before closing.

Reports live in `dev/dev-log/after-task/` (task-scope) or
`dev/dev-log/after-phase/` (phase-scope). Each report includes the goal,
files changed, checks run, consistency audit, tests-of-the-tests when
tests changed, what didn’t go smoothly, team learning, known
...
- **Track B** (Claude / Codex / AI): R-side function walk and vignette
  deep-dive. Owns `R/`, `tests/`, `vignettes/`, `dev/audit/`,
  `dev/dev-log/`, `_pkgdown.yml`, `NEWS.md`, `README.md`. Plan in
  `~/.claude/plans/purring-honking-dove.md`.

When handing work to another agent (or to the user, or back to Sergio),
leave enough context in `dev/dev-log/check-log.md`, an after-task
report, an issue, or a pull request for the next agent to continue
without rediscovering the whole problem.

Read-only sidecar perspectives (for design review, mathematical review,
documentation review, validation planning) are fine. Write- capable
workers should have narrow file ownership that doesn’t overlap with
other active work.
...
Product, architecture, or validation-policy changes need a normal task
...
- Don’t bypass `JuliaConnectoR` — there should be exactly one R↔︎Julia
  ...
  [`run_alife()`](https://itchyshin.github.io/clade/reference/run_alife.md)
  call.
- Don’t add a spec field to
  [`default_specs()`](https://itchyshin.github.io/clade/reference/default_specs.md)
  without a Julia consumer (`test-spec-wiring.R` will fail). If the
  field is a reserved-future placeholder, add it to
