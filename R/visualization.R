...
get_movement_data <- function(env) {
  # CHANGED: Look at the top-level env list, not inside env$specs!
...
#' Plot Run Movie
#' @export
plot_run_movie <- function(env, ticks = NULL, fps = 20, colour_by = "energy") {
  if (!requireNamespace("ggplot2", quietly = TRUE) || 
      !requireNamespace("gganimate", quietly = TRUE)) {
    stop("Packages 'ggplot2' and 'gganimate' are required for this function.")
  }
  
  df <- get_movement_data(env)
  
  if (!is.null(ticks)) {
    df <- df[df$tick %in% ticks, ]
  }
  
  p <- ggplot2::ggplot(df, ggplot2::aes(x = x, y = y, color = .data[[colour_by]], group = id)) +
    ggplot2::geom_point(size = 3) +
    ggplot2::theme_minimal() +
    ggplot2::coord_fixed(
      xlim = c(1, env$specs$grid_cols), 
      ylim = c(1, env$specs$grid_rows)
    ) +
