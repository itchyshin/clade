...
BNN-sigma-from-heterozygosity coupling has been the central mechanism
in four separate audit findings worth stating together:
...
1. **Pre-0.4.0 (🔴 Baldwin).** In the earliest kernel, sigma was
   ...
   canalization) fought against the behavioural-variance cost, and
   the net direction was wrong. Addressed by 0.5.5 / 0.5.6 with the
...
2. **Pre-0.5.10 (silent haploid conversion).** `_find_mate`
   ...
   `is_haploid(g) == true` → `sigma = fill(bnn_sigma_init, n)` for
   all weights, regardless of selection. The BNN-sigma-from-
   heterozygosity expression was wired correctly but the ploidy
   pipeline upstream was secretly converting everything to haploid.
   Fixed in 0.5.10 (`_find_mate` rewrite + `mate_search_radius` spec).
3. **Post-0.5.10 direction.** With the ploidy fix in, `mean_prior_sigma`
   now evolves from `bnn_sigma_init` to a selection-drift equilibrium
   (~0.33). Direction however is REVERSED from the canonical DeWitt /
   Hinton-Nowlan prediction: under tested parameters, stable envs
   maintain *more* heterozygosity than seasonal envs (t = −1.41
   direction, stable > seasonal). Either clade's kernel produces
   genuinely different dynamics than DeWitt 2004 (for reasons worth
   understanding), or `seasonal_amplitude = 0.5` bottlenecks purge
   variance faster than stable selection preserves it.
4. **Current status.** s-plasticity and s-baldwin remain 🟠 with a
   specific, well-understood diagnosis rather than "kernel
   unknowable". The BNN-sigma mechanism is no longer a kernel bug;
   it's a kernel property whose predictions we can now state
   honestly. (The additive expression for ANN/CTRNN/GRN brains is
...
✅ pre-0.5.10 but re-audited to 🟠 under the fixed diploid-sex
kernel: baseline diversity = 0.263 ± 0.009, hypermut diversity =
0.263 ± 0.009 (Δ = +0.000 at 4 seeds, grass_rate = 0.06). Real
diploid sex produces enough baseline mutation input at default
`mutation_sd = 0.1` that hypermutation doesn't add detectable
diversity. Pre-0.5.10 the baseline was structurally haploid
(single-parent gamete + mutation) which had lower effective
mutation input; that made hypermutation look like it was adding
something. With real diploid sex, the baseline is already at the
hypermutation level.
...
once on each parent to get the two haplotypes, then assembling them
into a `DiploidGenome`. When `ploidy == 2` but no mate is available
...
- **FALSE** (default): `pat_w = Float32[]`. The resulting
  ...
  effectively haploid even though it was produced by a ploidy-2
  agent.
- **TRUE**: draw the second haplotype from parent1 via meiosis
  (self-fertilization). The offspring remains diploid with both
  alleles from the same parent.
...
3. Outcross at longer range (long-distance dispersal of pollen,
   sperm, etc.).

clade's default is effectively **(1 with a twist)**: the offspring
is produced but collapses to haploid ploidy, which in downstream
code acts like a species switch mid-reproduction.

**Why this matters — the silent-haploid bug.** Pre-0.5.10,
`_find_mate` returned `nothing` whenever `signal_dims == 0` (no
mate-choice trait). That short-circuit meant `parent2` was
`nothing` for *every* reproduction in default-config scenarios.
Every offspring ended up with `pat_w = Float32[]`. The whole
diploid pipeline was a no-op: `is_haploid(g) == true` forever,
`make_bnn_brain` took the haploid branch, sigma was pinned at
`bnn_sigma_init`, heterozygosity-dependent inheritance (Hamilton's
pedigree r, Baldwin canalization, sex-vs-asex comparisons) was
all measuring haploid dynamics under a diploid label.

**Fixed in 0.5.10** by rewriting `_find_mate` to actually return
mates at default `signal_dims = 0` (random neighbour, sexual
reproduction without mate choice).
`self_fertilization_fallback` was added in 0.5.9 as a
complementary opt-in: even if the mate search fails, keep the
...
(`s-stress-hypermutation`, above). The bug was silently
invalidating audits for years; the re-audit confirms the
surviving claims are defensible under real diploid dynamics.

**Variants worth considering.**

- **Broader mate search** (`mate_search_radius = 2` or `3`)
  reduces Allee-failure rate without requiring the selfing
  fallback. Ecologically reasonable default for most species.
- **Outcrossed fallback** (not implemented): random cross-grid
  donor when local search fails, avoiding both haploid
  conversion and selfing-inbreeding. Probably the best long-term
  design.
- **Sex-vs-asex calibration**: mating-systems found that real
  diploid sex is viability-negative at current `repro_cost`
  settings (20/32 seeds crashed in the 0.5.10 re-audit). The cost
  structure was implicitly tuned for diploid-in-name-only
  (i.e. asex). Deciding what sex *should* cost is a separate
  design exercise.
