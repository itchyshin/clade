---
title: "Your first research project with clade"
author: "Shinichi Nakagawa"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Your first research project with clade}
...
plot. Read this after [Getting started](getting-started.html) when you
want to move from "ran the baseline" to "produced a result I could
cite".*
...
parental or kin-based provisioning essential. So the empirical
question becomes: does helping evolve *more readily* when the
neonatal deficit is active?

This is the kind of mechanistic question clade is built for. Here's
how you'd answer it.
...
[Scenarios](scenarios.html) and find the closest match to your
question. For our kin-selection question, start with
[`s-kin`](s-kin.html) — which already reproduces Hamilton's rule under
default conditions.
...
[Parameter reference](parameter-reference.html) for the exact flag
name:
...
A single run is never evidence. Use `batch_alife()` with at least
8 seeds for a pilot, 16–64 for publication. Keep runs short while
you debug, then scale up once the pipeline is working.
...
**Important:** mclapply and the Julia kernel don't compose — PSOCK
is the safe choice. `batch_alife()` handles this internally; just
set `n_cores`. If you see *"all connections are in use"*, drop
`n_cores` below 64.
...
If `viability_report()` flags failures, your specs are probably too
harsh (mortality too high, food too scarce) or inconsistent (disease
on without recovery rate, kin selection without diploidy). See
[Troubleshooting](troubleshooting.html) for common fixes.

---

## Step 5 — Compute the differential

Hamilton's rule is about *cooperation evolving* — measure the
frequency of helping-tendency alleles at the end of each run, and
...
For a Tier-C audit-style report, you want **|t| > 2 at 16+ seeds**
in the predicted direction. If you see |t| < 2 with a pilot of 8,
scale to 16; if it's still borderline, scale to 32. If the
direction is wrong, the model is telling you something — don't just
chase a bigger sample.
...
Save with `ggsave("hamiltons_rule_neonatal_deficit.pdf", width = 5,
height = 4)`.
...
> heritable-genome agents with kin selection, cooperative tendency,
> and parental care enabled. To test whether the neonatal foraging
> deficit (Aiello & Wheeler 1995) promotes the evolution of helping,
> we ran two conditions — `neonatal_foraging_deficit = 0.5` (altricial
> offspring) vs. 0 (no deficit) — each for 8 seeds of 500 ticks. Final
> helper-tendency allele frequencies differed by Δ = +X.XX ± Y.YY
> (t = +Z.Z; p < 0.05), supporting the prediction that altriciality
...
And — importantly — cite the biological theory you're testing
(Hamilton 1964; Aiello & Wheeler 1995). clade is the instrument;
the theory is the question.

---

## What changed from Getting started?

[Getting started](getting-started.html) gets you to "ran the
baseline". This walkthrough added:
...
- **Multi-seed sweep**: `batch_alife()` with ≥ 8 seeds; scale up for
  publication.
- **Viability check**: never interpret a simulation that didn't
  actually persist.
...
For the next level — parameter sweeps, MAP-Elites, CMA-ES — see
the [Parameter search](ps-introduction.html) guides. For
reproducing a specific published prediction, start with
[the paper-reproduction template](paper-template.html).

---

## Common variations of this workflow

- **Compare three conditions** (e.g. low / medium / high deficit):
  three specs lists, three `batch_alife()` calls, an ANOVA or a
  continuous regression. Use `seasonal_amplitude` if you want a
  continuous environmental gradient.
- **Ask a parameter-search question** ("*what deficit intensity
  maximally selects for helping?*"): use `search_cmaes()` with the
  helper frequency as the fitness function.
- **Explore a trait space** ("*what combinations of deficit, care,
  and clutch size produce helping?*"): use `search_map_elites()` to
  tile the trait space.
