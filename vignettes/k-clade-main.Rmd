---
title: "Clade.jl — the main loop, in biological order"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Clade.jl — the main loop, in biological order}
...
Source file: [`inst/julia/src/Clade.jl`](https://github.com/itchyshin/clade/blob/main/inst/julia/src/Clade.jl#L336)
(lines 336–414).
...
(documented in detail in [tick.jl chapter](k-tick.html)). Then run the predator
update loop.
...
Each predator does the same plus attack logic. See the [tick.jl chapter](k-tick.html) for the
full breakdown.
...
- **Per-agent decisions.** Those are in `tick_agents!`
  ([tick.jl chapter](k-tick.html)).
- **Reproduction logic.** That's in `create_offspring!`
  ([reproduce.jl chapter](k-reproduce.html)).
- **Death decisions.** Those are in `kill_dead!`
  ([death.jl chapter](k-death.html)).
- **Sensing.** That's in `sense_agent` ([sense.jl chapter](k-sense.html)).
