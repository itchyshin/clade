...
`alifeR` embedded the simulation hot path in Rcpp, `clade` runs the entire
simulation in Julia (via
[JuliaConnectoR](https://CRAN.R-project.org/package=JuliaConnectoR)). The
R–Julia boundary is crossed *once* per `run_alife()` call, not once per
...
derivative-free optimisation (CMA-ES), neither of which is feasible at
this scale from an Rcpp simulator.

This vignette is an orientation, not a reference. It introduces the
workflow, the parameter list, the brain types, and the biological modules
that can be toggled on or off. All code chunks below are shown but not
evaluated, because the Julia backend is not available during package check
or vignette build.
...
On the **first** call to `run_alife()`, `clade` starts a Julia session,
installs its own Julia dependencies into a private environment, and
precompiles the simulation package. This takes 60–90 seconds and is cached
for all subsequent runs in the same Julia environment.
...
- Set `JULIA_BINDIR` in your `.Renviron` file to the directory containing
  the `julia` binary.
...
`env` is a `clade_env` object. Its `print` method gives a one-line summary:
...
```{r fig-run-dashboard, eval=TRUE, echo=FALSE, out.width="100%", fig.alt = "plot_run() produces a six-panel dashboard", fig.cap="plot_run() produces a six-panel dashboard: population size, mean energy (±SD ribbon), genetic diversity, births and deaths per tick, grass coverage, and BNN prior sigma."}
...
- `data$ticks` — one row per logged tick, with population-level summaries
  (mean energy, mean body size, genetic diversity, grass cover, births,
  deaths, and a column per active module).
- `data$deaths` — one row per agent death, with cause, age, energy, final
  body size, and lifetime offspring count.
...
`default_specs()` returns a long named list (~296 parameters in
0.7.0). To see only what you have changed, use
`print_specs(diff_only = TRUE)`:
...
`vignette("parameter-reference")` or the help page for `?default_specs`.

---

## Simulation timescale: choosing the right spec preset

The single most important choice when designing a clade run is
**how many generations you will simulate**. Fisher (1930) showed
that a beneficial allele needs roughly \(1/s\) generations to fix,
where \(s\) is the selection coefficient. For modest selection
(\(s \approx 0.05\text{–}0.10\)), this is 20–100 generations.
Weaker selection needs hundreds.

Under `default_specs()`, generation time is roughly
`max_age / 2 ≈ 100 ticks` plus maturation delay, so the effective
generation time is ~190 ticks. A 500-tick run is therefore only
~2.6 generations — too short to see any evolutionary dynamics at
realistic selection strengths. This is the single largest source
of "weak effect" reports in early clade audits.
...
|---|---|---|---|---|
| `fast_specs()` | ~30 ticks | 2000 | ~66 | Any evolutionary scenario — plasticity, Baldwin, body size, cooperation, speciation |
| `default_specs()` | ~190 ticks | 500 | ~2.6 | Within-generation demos — ecology, predator-prey, disease, learning, kitchen-sink |
| `slow_specs()` | ~200 ticks | 10000 | ~50 | K-strategist scenarios (elephant, whale) where long life history is the point |

`fast_specs()` achieves a short generation time by lowering
`max_age` to 30 ticks and `min_repro_energy` to 60 (versus 200 and
120 in default). This is how the MATLAB ancestor of clade
(Bulitko 2023) ran — agents matured quickly and turned over fast,
...
**Rule of thumb.** If the phenomenon you want to see requires
*allele change* (heritable trait evolution, genetic assimilation,
cooperation or signal elaboration), use `fast_specs()`. If it is a
demographic or ecological phenomenon that unfolds within a single
generation (predator-prey cycles, disease spread, seasonal grass
dynamics), `default_specs()` is fine.

See [`dev/docs/timescale-analysis.md`](https://github.com/itchyshin/clade/blob/main/dev/docs/timescale-analysis.md)
for the biological reference table (E. coli through elephant) and
the full MATLAB-ancestor comparison that motivated these presets.
...
others live in `?default_specs`. Values shown are the 0.7.0 defaults.
...
Mouret & Clune 2015) cleanly. In 0.4.0, the BNN's prior-width can be drawn
from heterozygosity (legacy), a fixed value, or the evolved
`plasticity` trait — see `bnn_sigma_source` in `?default_specs`.
...
surcharge; smaller agents eat less but are cheaper to run. Reference size
1.0 has no metabolic correction. The trait is inherited via the genome and
bounded by `body_size_min` (default 0.3) and `body_size_max` (default 3.0).
...
`batch_alife()` runs a list of specs across R worker processes. This is
the recommended way to compare parameter regimes, since each Julia process
stays warm for its slice of the batch:
...
search), see the [parameter search guide](ps-introduction.html).
...
The first call in an R session pays a one-off start-up cost (~60–90 s) for
Julia compilation. Subsequent calls reuse the warm Julia session and
...
- **Biological scenarios.** `vignette("scenarios")` is a discovery guide
  to all 36 pre-packaged scenarios, each with runnable code, expected
  dynamics, and follow-up experiments.
- **Parameter search.** The [parameter search introduction](ps-introduction.html)
  explains when to use random search, MAP-Elites, CMA-ES, or viability
  mapping. Separate guides cover
  [agent-level parameters](ps-agent-parameters.html),
  [environment-level parameters](ps-environment-parameters.html), and
  [the algorithms themselves](ps-algorithms.html).
- **Full parameter reference.** Every parameter in `default_specs()` is
  documented in `vignette("parameter-reference")`.
- **Reading the kernel as biology.** The
  [kernel-as-biology](k-README.html) series translates the Julia
...
- **Paper reproductions.** `vignette("paper-template")` plus six
  worked examples (K&B 2003, Griesser 2023, D&D 1999, Réale 2010,
  Emlen 1982, Courchamp 1999) show how to reproduce a published
  prediction in clade end-to-end.
- **0.4.0 kernel changes.** The
  [0.4.0 changelog](https://github.com/itchyshin/clade/blob/main/dev/docs/kernel-0.4.0.md)
