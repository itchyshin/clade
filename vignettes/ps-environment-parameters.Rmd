---
title: "Parameter search — environment-level parameters"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Parameter search — environment-level parameters}
...
are typically what gets tuned when you ask "in which world does this kind
of organism thrive?". For per-organism parameters (traits, brain
architecture, life history, reproduction), see
[ps-agent-parameters.html](ps-agent-parameters.html).

This vignette lists the environment-level parameters by domain, flags the
ones that are only active under an enabling flag, and shows worked
searches — especially `search_viability()` for mapping survivable regions
before committing to CMA-ES. For the algorithms themselves, see
...
workflow uses short `max_ticks` values (200–500) for the search and
long `max_ticks` (1000+) for the winning configuration's final
verification.
...
| `fixed_patch` | `NULL` | Named list defining a perennially-rich cell (see `?default_specs`) |
...
little productive area, or wrong energy ratios), and CMA-ES cannot follow
a gradient through a dead region. Use `search_viability()` first — see
...
## External forces — predators {#predators}
...
The per-predator traits (`predator_attack_strength`, `predator_energy_gain`)
are closer to agent parameters — see
...
## Worked example: viability mapping {#viability-mapping}
...
doesn't go extinct. `search_viability()` does this in a single call and
returns both a heatmap and a tidy data frame.
...
`tune_complex_landscape()` is a pre-configured wrapper: it sets five
landscape parameters (`shrub_density`, `canopy_density`, `shrub_energy`,
`canopy_energy`, `shrub_growth_rate`) and uses
`objective_complex_landscape()`, which measures the joint increase in
wing size, niche diversity, and survival.
...
- **[ps-introduction.html](ps-introduction.html)** — when to search,
  which algorithm, how to design an objective function.
- **[ps-agent-parameters.html](ps-agent-parameters.html)** — traits,
