...
# 0.8.0: 22 → 24 (TRAIT_AGING_RATE_FEMALE_GENE, TRAIT_AGING_RATE_MALE_GENE
# for the sex-specific trait expression mechanism).
test_that("N_SCALAR_TRAITS is defined as 24 in types.jl", {
  skip_no_julia_src()
  types_jl <- paste(readLines(file.path(JULIA_SRC, "types.jl")), collapse = "\n")
  expect_true(grepl("N_SCALAR_TRAITS = 24", types_jl, fixed = TRUE))
})

# ── 10. Trait indices are all distinct 1..24 ───────────────────────────────────
test_that("TRAIT_* indices in types.jl are distinct and cover 1..24", {
  skip_no_julia_src()
  types_jl <- readLines(file.path(JULIA_SRC, "types.jl"))
  trait_lines <- grep("^const TRAIT_", types_jl, value = TRUE)
  expect_equal(length(trait_lines), 24L,
               info = paste("Expected 24 TRAIT_ constants, got", length(trait_lines)))
  indices <- as.integer(sub(".*= (\\d+)$", "\\1", trait_lines))
  expect_equal(sort(indices), 1L:24L)
})
