...
[`dev/audit/fidelity/paper_mcelreath_2007.R`](https://github.com/itchyshin/clade/blob/main/dev/audit/fidelity/paper_mcelreath_2007.R)
(3 horizons × 8 seeds = 24 runs, ~24 min wall-clock). Saved to
[`dev/audit/fidelity/paper_mcelreath_2007.rds`](https://github.com/itchyshin/clade/blob/main/dev/audit/fidelity/paper_mcelreath_2007.rds).
Headline finding: **the peak-then-decay trajectory disappears at 8
seeds.**
...
- The single-seed +0.307 at 5 000 ticks is a noise-envelope landing on
  the positive tail; the multi-seed mean is **−0.077** — opposite sign.
- The 15 000-tick “decay to +0.032” is also single-seed noise; mean is
  +0.034.
- The multi-seed pattern is **null at every horizon** (none reach 2σ
  PASS). There’s no peak to decay from.
...
**Surprise positive finding**: at the *shortest* horizon (2 000 ticks),
`cor(exp, aggro)` is **−0.172 ± 0.042, t = −4.12 → PASS-negative**. This
is the only multi-seed cell across all three Wolf 2007 reproductions in
clade (Wolf 2007 + Massol-Crochet + McElreath) that produces a 2σ PASS
in the predicted direction. Notably, this is *opposite to McElreath’s
prediction*: the asset-protection signal is strongest at the *earliest*
time-point, not later. If asset-protection is a real mechanism in
clade’s spatial kernel, it appears early and is washed out by 5 000
...
([`dev/audit/fidelity/paper_mcelreath_2007_16seed.rds`](https://github.com/itchyshin/clade/blob/main/dev/audit/fidelity/paper_mcelreath_2007_16seed.rds)):
mean = **−0.100 ± 0.031, t = −3.26, PASS-negative**. The mean magnitude
dropped (−0.17 → −0.10) but the SE tightened so the verdict survives.
**The asset-protection signal in `cor(exp, aggro)` at 2 000 ticks is a
robust positive finding** — confirmed at twice the sample size. Notable:
clade’s spatial Wolf 2007 *does* produce an asset-protection signal, but
it appears in `cor(exp, aggro)` at the early end of the time range,
opposite to McElreath’s “later-peak” prediction.

### Trait-mean stability check

`mean(exploration)` across horizons: 0.119 → 0.112 → 0.113. The
population *is* in a stable equilibrium structure at all three horizons.
So the disappearing syndrome cannot be blamed on equilibrium drift — the
equilibrium is there; the syndrome simply doesn’t emerge across seeds.

## Reading the result — honest revision

The original single-seed reading was: “McElreath’s critique is
**strongly borne out** — the syndrome peaks at 5 000 ticks then decays.”
At 8 seeds, **neither McElreath’s transience prediction nor Wolf’s
syndrome prediction is robust**: the bold-aggro syndrome is null at
every horizon tested. Both authors’ opposing claims about that syndrome
are *equally untestable* at this sample size — there’s no signal to be
transient or persistent.

The one robust multi-seed finding is **opposite to McElreath**:
`cor(exp, aggro)` shows asset-protection at the *shortest* horizon (2
000 ticks, PASS-negative), not the longest. The mechanism may be real
but presents in the asset-protection correlation rather than the
cross-context syndrome, and on the early-decay side of the time-axis
rather than the late-decay side.

Recommended interpretation: clade’s spatial Wolf 2007 reproduction is
null on the syndrome metric at 8 seeds, so the McElreath transience
critique cannot be tested rigorously without first strengthening the
underlying signal (longer runs, larger population, kernel changes — or
perhaps the cor(exp, aggro) is the actual robust asset-protection signal
and the syndrome metric was a red herring). Documented as a candidate
finding for the next kernel-review pass.
...
Reading
[`vignette("paper-massol-crochet-2008")`](https://itchyshin.github.io/clade/articles/paper-massol-crochet-2008.md)
and this vignette together at the multi-seed level (8 seeds each, both
rds saved):
...
- **Massol & Crochet 2008**: the single-seed claim “peak at β = 1.25”
  *flips at 8 seeds* — β = 1.25 is the trough (mean = −0.077,
  ...
  syndrome.
- **McElreath et al. 2007** (this vignette): the single-seed claim “peak
  at 5 000 ticks decaying to near-zero by 15 000” flips at 8 seeds —
  bold-aggro is null at every horizon; there’s no peak to decay from.

Both critiques’ apparent confirmations from single seeds were noise-
envelope landings, not real signals. The cumulative multi-seed picture
across all three Wolf 2007 reproductions in clade (`paper-wolf2007`,
`paper-massol-crochet-2008`, `paper-mcelreath-2007`) is that **the
boldness-aggressiveness syndrome does not robustly emerge** in clade’s
spatially-explicit kernel at any (β, horizon) combination tested.
Documented as a candidate finding for the next kernel-review pass.
