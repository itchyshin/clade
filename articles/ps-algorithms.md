...
**When.** You want the single best parameter combination for one scalar
objective, and the parameters are continuous (not discrete categorical
choices like `brain_type`).
...
different seeds). Doesn’t handle categorical parameters (`brain_type`,
`life_history`).

**Rule of thumb for choosing between CMA-ES and MAP-Elites:** use CMA-ES
when you care about *where the optimum is*; use MAP-Elites when you care
about *what the landscape looks like around the optimum*. CMA-ES is
faster per run for a single-objective answer; MAP-Elites is more
...
**Weaknesses.** Only 2 parameters at a time (by design — heatmap
output). Doesn’t optimise; just shows you where it’s safe to optimise.

### Pre-packaged tuners

For the two scenarios where viability mapping is particularly
...
**What it does.** Estimates the gradient of the objective with respect
to each parameter by finite differences, then takes a step in the
positive direction. Repeats until the step size drops below a threshold.
