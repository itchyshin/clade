...
**What we found.** Running with `brain_type = "bnn"`, 80 agents, 25×25
grid, `grass_rate = 0.15`, 600 ticks (seed 42): `mean_prior_sigma` rose
monotonically from 0.30 at ticks 1–50 to 0.50 (the maximum ceiling) by
tick 300, where it remained for the rest of the run. Sigma moved
*upward*, not downward — the opposite of Baldwin Effect canalization.

This result is not a failure of the implementation. It is a
theoretically coherent finding: in a spatially heterogeneous environment
with no single optimal foraging policy, maintaining exploration is
adaptively superior to fixing any particular strategy. The fitness
landscape is not stable enough for canalization to proceed. Canalization
requires a fitness landscape with a stable global optimum — a condition
that a dynamic foraging world with population competition does not
provide at short run lengths.

**Theoretical interpretation.** Mayley (1996) showed that the Baldwin
Effect operates fastest when (1) the fitness landscape has a single
sharp peak, (2) mutational variance is sufficient for selection to find
it, and (3) generation times are long enough for selection to act. In a
competitive foraging world with spatial heterogeneity and stochastic
resource patches, condition (1) fails: the optimal foraging strategy
shifts continuously with population density and grass distribution.
Selection therefore maintains the ability to learn (high σ) as the
