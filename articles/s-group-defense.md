...
![Expected output: group defense (green) sustains a larger and more
stable population than the baseline (red) under identical predation
pressure, illustrating the dilution-of-risk
effect.](figures/showcase_15_group_defense.png)

Expected output: group defense (green) sustains a larger and more stable
population than the baseline (red) under identical predation pressure,
illustrating the dilution-of-risk effect.

**What we found.** Running with 15 predators
(`predator_attack_strength = 60`), 80 agents on a 30×30 grid, 400 ticks
(3 replicates, seeds 41–43): group defense produced a +5 agent mean
population advantage (402 vs 398) and reduced total deaths by a mean of
2 per run — differences within stochastic noise. The damage-reduction
mechanism is correctly implemented: at population density ≈ 0.4
agents/cell, each agent has an expected 9 neighbours within the radius-2
Moore neighborhood, giving a damage factor of 1/(1 + 9 × 0.3) ≈ 0.27, a
theoretical 73% reduction in attack damage. The reason the population
effect is negligible is that at these densities, energy-based mortality
(starvation when grass is locally depleted) dominates over predation
mortality; removing 73% of predation damage cannot substantially change
a death rate already driven by food scarcity. Hamilton’s (1971) selfish
herd advantage is most detectable when a sparse founding population
transitions from isolation to grouping — a dynamic best captured by
starting with `n_agents_init = 10` and comparing cumulative survival
through the founding bottleneck phase.
