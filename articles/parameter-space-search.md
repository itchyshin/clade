...
# MAP-Elites: quality-diversity search (serial — the MAP-Elites
# loop is inherently sequential because each iteration's parent is
# sampled from the current archive)
...
parallelises the candidate evaluation per generation across the PSOCK
path introduced in 0.5.6.
[`search_map_elites()`](https://itchyshin.github.io/clade/reference/search_map_elites.md)
is inherently serial (each iteration’s parent depends on the current
archive); if you want to fan-out-run multiple MAP-Elites *searches* in
parallel, wrap them in
