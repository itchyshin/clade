...
**Heritable-feedback finding (2026-04-19 ✅ at
`shelter_occupancy_bonus > 0`).** The canonical Odling-Smee et al. 2003
claim — that niche construction creates a *heritable environmental
feedback* rewarding the builders’ lineage — is tested by sweeping
`shelter_occupancy_bonus` across {0, 1, 3, 5} at 8 seeds each with
`niche_construction = TRUE` + 5 predators. Key result at 40 runs:

| `shelter_occupancy_bonus` | final_n ± SE | Δ vs bonus=0     | t              |
|---------------------------|--------------|------------------|----------------|
| 0 (mechanism only)        | 179.9 ± 3.2  | (baseline)       | —              |
| 1                         | 216.9 ± 4.8  | **+37.0 ± 5.8**  | **+6.39 PASS** |
| 3                         | 243.2 ± 6.7  | **+63.3 ± 7.4**  | **+8.54 PASS** |
| 5                         | 262.2 ± 10.3 | **+82.3 ± 10.8** | **+7.65 PASS** |

Spearman(bonus, final_n) = **+0.863**. Notably, `bonus = 0` gives
*lower* population (179.9) than the no-construction control (188.3) —
shelters slow grass regrowth, so the construction mechanism alone is a
net cost. The **heritable feedback** (bonus \> 0) is what flips niche
construction from private cost to large population-level benefit,
exactly as Odling-Smee et al. 2003 argue. Full protocol:
