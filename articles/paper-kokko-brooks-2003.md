...
Four resource levels crossed with signals on/off, 8 seeds each: 64 runs
at 40 × 40 grid, 2000 ticks, no predators (to isolate resource-driven
stress from external mortality).

**Signals-on arm**: `signal_dims = 3L`, `signal_cost = 0.2` (above the
drift-scale noise documented in s-signals.Rmd),
`mate_choice_mode = "preference"`, `mate_choice_strength = 0.7`.
**Signals-off arm**: `signal_dims = 0L`, `mate_choice_mode = "random"`.

**Resource levels**: `grass_rate ∈ {0.20, 0.12, 0.08, 0.05}` spanning
abundant → very_scarce.
