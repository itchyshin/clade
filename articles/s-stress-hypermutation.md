...
make_s <- function(hypermut) {
  # default_specs() — fast_specs + grass_rate = 0.05 crashes
  # completely (n_final ≈ 0 per crash_audit.R). The stress
  # hypermutation regime needs low grass, but fast_specs short
  # lifespan can't survive low grass.
  s <- default_specs()
