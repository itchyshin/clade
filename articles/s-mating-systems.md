...
1.  **Continuous-trait parasites *widen* the sex-asex gap** rather than
    ...
    Queen doesn’t work.
2.  **Discrete-allele parasites produce the canonical direction.** Under
    the 0.5.1 module
    (`n_parasite_loci = 16, pressure = 2.0, exponent = 6, mutation = 0.02`),
    sex beats asex by Δn = +1.1 at audit-default parameters. A tuning
    grid found regimes up to Δn = +7.7. The first observation of sex \>
    asex in clade, and the Hamilton 1980 direction.

The diversity metric (Δdiv) still shows sex below asex — a measurement
...
Verdict: 🟠 with canonical Red Queen direction confirmed at moderate
magnitude. Stronger pressure or larger populations could promote to ✅;
scenario remains parameter-sensitive.
