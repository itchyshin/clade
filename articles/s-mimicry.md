...
**Expected output (audited 2026-04-15).** Across multi-seed runs at ten
different parameter regimes — from defaults to extreme cost = 0, dose =
...
`prey.toxicity` directly, while alifeR’s R prototype uses a **signal
vector** updated toward `prey.signal`. The scalar simplification removes
signal-specific learning — predators cannot recognise warning
*patterns*, only their general aversion level — which is the mechanism
Bates (1862) and Müller (1879) actually described. Predators therefore
rarely learn to avoid toxic prey (max 9 avoidance events / 600 ticks at
extreme settings), and without selective avoidance, toxic prey gain no
measurable survival advantage. Restoring signal-vector predator memory
is on the 0.4.0 backlog.
