# genome.jl — meiosis, traits, inheritance

## `genome.jl` — meiosis, traits, and inheritance

This is the biggest file in the kernel (~540 lines). It implements the
**genetic layer** of the simulation — the structures and operations that
determine how heritable information passes from parents to offspring.

clade’s genome model is unusually elaborate for an evolutionary ABM: it
implements proper diploid meiosis with chromosomes, recombination,
...
Source file:
[`inst/julia/src/genome.jl`](https://github.com/itchyshin/clade/blob/main/inst/julia/src/genome.jl)
(~540 lines).
...
(Defined in
[`types.jl`](https://github.com/itchyshin/clade/blob/main/inst/julia/src/types.jl),
reproduced conceptually here.)
...
**What this says.** Each agent has a genome consisting of two “strands”
of brain weights and two strands of scalar traits, plus metadata
(network architecture, chromosome count). For haploid agents
...
The 15 scalar traits are heritable phenotypic traits independent of the
brain — body size, metabolic rate, aging rate, cooperation level,
dispersal tendency, mutation rate, plasticity, etc. These give clade a
richer genome than just “brain weights” — biological scenarios that need
a heritable life-history trait have one to evolve.

The `n_chromosomes` parameter sets how many independently-recombining
segments the brain genome is split into. With `n_chromosomes = 1`, all
weights recombine together (low effective recombination). With
...
flag is on. If on, sample an initial value from a truncated normal (mean
= init, sd = mutation_sd, clamped to \[min, max\]). If off, use a fixed
default value (often 1.0 or 0.0).

**Biology.** This is **founder population sampling**. The initial agents
have heritable variation in any traits the user has activated; unused
traits are at their “biologically neutral” default (e.g., metabolic rate
1.0 = baseline, body size 1.0 = reference, cooperation 0.0 = no
cooperation).

The truncated normal is a standard model for biological trait
distributions — most traits cluster around a mean, with some spread, and
have biological min/max bounds (you can’t have negative body size).

**Audit findings.** Two scenarios flagged this layer:

- **s-mutation-rate** (no audit yet): the `TRAIT_MUTATION_SD` field is
  evolvable when `mutation_rate_evolution = TRUE`, allowing populations
  to evolve their own mutation rate (Sniegowski et al. 1997).
- **s-toxicity** (in mimicry audit): `TRAIT_TOXICITY` was found to be
  surprisingly insensitive to selection at default parameters — related
  to the scalar predator memory issue, not to this trait initialisation.
...
1.  **Independent assortment** — each chromosome chooses a parent strand
    independently. This is what Mendel discovered in pea plants (his
    second law).
2.  **Recombination / crossover** — within a chromosome, the gamete
    alternates between the two parental strands at random crossover
    points. This is what shuffles linked genes within a generation. The
    Poisson model with rate `crossover_rate` is the standard
    continuous-time approximation.
3.  **Mutation** — after recombination, Gaussian noise is added to every
    weight. This generates the new variation that natural selection acts
    on.

This is the most **biologically realistic** part of the kernel and a
clade-specific improvement over alifeR (which used flat-weight crossover
with no chromosome structure).
...
inheritance is simpler: each trait independently chooses one of the two
parent alleles, then gets mutated. No chromosomes, no recombination —
each trait is its own unlinked locus.
...
The simplification is biologically defensible because the scalar traits
are *interpretable* (each maps to a specific phenotype documented in
`vignettes/parameter-reference.Rmd`). If they were linked, evolutionary
correlations would be confounded.
...
**What this says.** Convert a diploid genome (two strands) into a single
phenotype (one set of weights actually used by the brain). Three modes:

- **Haploid** — phenotype is just the maternal strand (no choice
  needed).
- **Dominant** — for each weight, randomly pick maternal or paternal.
  Implements the random-allele-expression interpretation of dominance.
...
**Biology.** **Dominance** is one of the central concepts of Mendelian
genetics. In real organisms:

- Some traits are dominant/recessive (one allele’s phenotype dominates).
...
loci (Falconer & Mackay 1996). Brain weights are explicitly quantitative
— additive is the right default.

The BNN brain has a **special interpretation** of the diploid genome:
the maternal and paternal weights are interpreted as “two estimates of
the optimal weight”, and the BNN sigma (uncertainty) is computed from
their *difference* (heterozygosity). This couples genetic diversity to
behavioural flexibility.

**Audit findings (updated 2026-04-18, 0.5.11).** The
BNN-sigma-from-heterozygosity coupling has been the central mechanism in
four separate audit findings worth stating together:

1.  **Pre-0.4.0 (🔴 Baldwin).** In the earliest kernel, sigma was
    computed from heterozygosity AND controlled both (a) action noise
    and (b) the effective learning rate in `bnn_update!`. Under this
    full coupling, stable-env selection for narrower sigma (Baldwin
    canalization) fought against the behavioural-variance cost, and the
    net direction was wrong. Addressed by 0.5.5 / 0.5.6 with the
    `bnn_action_noise_scale` and `bnn_sigma_lr_scale` specs that let
    these two channels be decoupled.
2.  **Pre-0.5.10 (silent haploid conversion).** `_find_mate`
    short-circuited on `signal_dims = 0`, so every default-config
    diploid run produced offspring with `pat_w = Float32[]` →
    `is_haploid(g) == true` → `sigma = fill(bnn_sigma_init, n)` for all
    weights, regardless of selection. The BNN-sigma-from- heterozygosity
    expression was wired correctly but the ploidy pipeline upstream was
    secretly converting everything to haploid. Fixed in 0.5.10
    (`_find_mate` rewrite + `mate_search_radius` spec).
3.  **Post-0.5.10 direction.** With the ploidy fix in,
    `mean_prior_sigma` now evolves from `bnn_sigma_init` to a
    selection-drift equilibrium (~0.33). Direction however is REVERSED
    from the canonical DeWitt / Hinton-Nowlan prediction: under tested
    parameters, stable envs maintain *more* heterozygosity than seasonal
    envs (t = −1.41 direction, stable \> seasonal). Either clade’s
    kernel produces genuinely different dynamics than DeWitt 2004 (for
    reasons worth understanding), or `seasonal_amplitude = 0.5`
    bottlenecks purge variance faster than stable selection preserves
    it.
4.  **Current status.** s-plasticity and s-baldwin remain 🟠 with a
    specific, well-understood diagnosis rather than “kernel unknowable”.
    The BNN-sigma mechanism is no longer a kernel bug; it’s a kernel
    property whose predictions we can now state honestly. (The additive
    expression for ANN/CTRNN/GRN brains is unaffected by all of this.)
...
with standard deviation `mutation_sd`. If `mutation_sd = 0`, do nothing.

**Biology.** **Per-locus Gaussian mutation** — the standard
quantitative-genetics model. Each generation, every weight is
independently perturbed by a small random amount. The cumulative effect
over many generations is what gives selection raw material to work with.
...
**Audit findings (2026-04-18 update).** s-stress-hypermutation was ✅
pre-0.5.10 but re-audited to 🟠 under the fixed diploid-sex kernel:
baseline diversity = 0.263 ± 0.009, hypermut diversity = 0.263 ± 0.009
(Δ = +0.000 at 4 seeds, grass_rate = 0.06). Real diploid sex produces
enough baseline mutation input at default `mutation_sd = 0.1` that
hypermutation doesn’t add detectable diversity. Pre-0.5.10 the baseline
was structurally haploid (single-parent gamete + mutation) which had
lower effective mutation input; that made hypermutation look like it was
adding something. With real diploid sex, the baseline is already at the
hypermutation level.

------------------------------------------------------------------------

### 6.5. Offspring-genome assembly — where ploidy can flip silently

``` julia
function make_offspring_genome(parent1::DiploidGenome,
                               parent2::Union{DiploidGenome, Nothing},
    ...
    elseif specs["ploidy"] == 2 && selfing
        pat_w = meiosis(parent1, specs, rng)
        pat_t = meiosis_traits(parent1, specs, rng)
    else
        pat_w = Float32[]      # <-- effectively haploid offspring
        pat_t = Float32[]
    end
    DiploidGenome(mat_w, pat_w, mat_t, pat_t,
                  parent1.architecture, parent1.n_chromosomes)
end
```

**What this says.** Produce an offspring genome by calling `meiosis`
once on each parent to get the two haplotypes, then assembling them into
a `DiploidGenome`. When `ploidy == 2` but no mate is available
(`parent2 === nothing`), the behaviour depends on
`self_fertilization_fallback`:

- **FALSE** (default): `pat_w = Float32[]`. The resulting
  `DiploidGenome` has an empty paternal side, which makes
  `is_haploid(g) == true` everywhere downstream — the offspring is
  effectively haploid even though it was produced by a ploidy-2 agent.
- **TRUE**: draw the second haplotype from parent1 via meiosis
  (self-fertilization). The offspring remains diploid with both alleles
  from the same parent.

**Biology.** Real organisms have at least three options when a mate
isn’t found:

1.  Skip reproduction this cycle.
2.  Self-fertilize (common in plants, hermaphrodite animals).
3.  Outcross at longer range (long-distance dispersal of pollen, sperm,
    etc.).

clade’s default is effectively **(1 with a twist)**: the offspring is
produced but collapses to haploid ploidy, which in downstream code acts
like a species switch mid-reproduction.

**Why this matters — the silent-haploid bug.** Pre-0.5.10, `_find_mate`
returned `nothing` whenever `signal_dims == 0` (no mate-choice trait).
That short-circuit meant `parent2` was `nothing` for *every*
reproduction in default-config scenarios. Every offspring ended up with
`pat_w = Float32[]`. The whole diploid pipeline was a no-op:
`is_haploid(g) == true` forever, `make_bnn_brain` took the haploid
branch, sigma was pinned at `bnn_sigma_init`, heterozygosity-dependent
inheritance (Hamilton’s pedigree r, Baldwin canalization, sex-vs-asex
comparisons) was all measuring haploid dynamics under a diploid label.

**Fixed in 0.5.10** by rewriting `_find_mate` to actually return mates
at default `signal_dims = 0` (random neighbour, sexual reproduction
without mate choice). `self_fertilization_fallback` was added in 0.5.9
as a complementary opt-in: even if the mate search fails, keep the
offspring diploid via selfing.
...
- **Broader mate search** (`mate_search_radius = 2` or `3`) reduces
  Allee-failure rate without requiring the selfing fallback.
  Ecologically reasonable default for most species.
- **Outcrossed fallback** (not implemented): random cross-grid donor
  when local search fails, avoiding both haploid conversion and
  selfing-inbreeding. Probably the best long-term design.
- **Sex-vs-asex calibration**: mating-systems found that real diploid
  sex is viability-negative at current `repro_cost` settings (20/32
  seeds crashed in the 0.5.10 re-audit). The cost structure was
  implicitly tuned for diploid-in-name-only (i.e. asex). Deciding what
  sex *should* cost is a separate design exercise.
...
- **Brain construction.** That’s in `brains/*.jl`. The genome contains
  weights; the brain *interprets* them. Different brain types decode the
  same weight vector differently (an ANN reads them as point estimates,
  a BNN reads them as posterior means).
- **Selection.** Selection is the result of differential survival and
  reproduction, not a separate module. Genomes that produce better
  foragers leave more copies; genomes that don’t, vanish.
...
1.  **Is meiosis correct?** Yes — independent assortment (Mendel’s 2nd
    law) per chromosome, Poisson-distributed recombination per
    chromosome, then per-locus Gaussian mutation. This is the textbook
    model.
2.  **Is dominance modelled?** Yes — additive (default), dominant
    (random allele expression), codominant (additive at phenotype
    level). No incomplete-dominance mode but it would be additive with a
    tunable midpoint.
3.  **Are scalar traits inherited correctly?** Each trait inherits from
    one parent allele independently — fine for unlinked loci. No
    chromosomal linkage between scalar traits.
...
- Charlesworth, B. & Charlesworth, D. (2010) *Elements of Evolutionary
  Genetics.* Roberts & Company.
