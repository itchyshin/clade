...
**What we found (updated 2026-04-17, strength sweep at fast_specs).**
The 2026-04-16 0.4.1 audit ran at `default_specs + max_ticks = 1000` and
reported Δ ≈ +0.006 (direction OK, below the 0.02 threshold). Re-running
at `fast_specs` (~66 generations instead of ~5) with
`habitat_preference_strength` swept cleanly reveals the promotion:

| strength      | 5-seed Δ ± sd      | min_n | Verdict                         |
|---------------|--------------------|-------|---------------------------------|
| 0.5 (default) | +0.003 ± 0.005     | 110   | near-zero                       |
| 1.0           | +0.007 ± 0.006     | 122   | direction PASS, below threshold |
| **2.0**       | **+0.021 ± 0.005** | 117   | **PASS, crosses 0.02**          |
| **4.0**       | **+0.027 ± 0.007** | 137   | **PASS, crosses 0.02**          |

At `habitat_preference_strength ≥ 2.0` the Fretwell-Lucas IFD direction
is robustly positive with magnitude exceeding the 0.02 ✅ threshold at 5
seeds — the mean is roughly 4× the within-seed standard deviation,
comfortably outside the noise floor.

Mechanism. The default strength = 0.5 means each preference move attempt
fires with probability `0.5 × |hp|`, which is `0.5 × 0.03 ≈ 1.5%` per
tick at typical evolved preference values — not nearly enough effective
movement-toward-grass to produce a fitness differential larger than
drift. At strength = 2.0 the effective move rate is 6% per tick per unit
preference, which does clear the noise floor.

Verdict 🟠 → **✅ promoted (2026-04-17)** at
`habitat_preference_strength = 2.0, fast_specs()`. Use
`habitat_preference_strength = 0.5` to reproduce the near-null result at
default parameters (below threshold but direction OK); use
`strength = 2.0` for the clean promotion regime.

The 2026-04-16 0.4.1 audit result (+0.006 at default_specs) is preserved
for reference as the “default-timescale” value; the promotion comes from
the combined `fast_specs` + `strength = 2.0` configuration, not from
either alone.
...
s$habitat_preference_strength    <- 2.0  # promotes to ✅; default 0.5 is near-null
